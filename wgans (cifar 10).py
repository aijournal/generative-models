...
if len(DATA_DIR) == 0:
    raise Exception("Specify path to data directory")
...
def calc_gradient_penalty(D,real_data, fake_data):
    ...
    gradients = autograd.grad(outputs=disc_interpolates, inputs=interpolate,
...
def generate_image(frame,G):
    ...
    samples = G(noisev)
    samples = samples.view(-1,2,32,32)
...
for iteration in range(ITERS):
    ...
    if False and iteration % 1000 == 999:
        inception_score = get_inception_score(G)
